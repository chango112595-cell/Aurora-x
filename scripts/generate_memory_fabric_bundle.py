...
def generate_manifest(stats: BundleStats, files_manifest: list[dict]) -> dict[str, Any]:
    ...
    return {
        ...
        "checksums": {"algorithm": "sha256", "verified": True},
    }
...
def main():
    ...
    if src_py.exists():
        count = copy_directory(
            src_py,
            py_dir / "aurora_memory_fabric_v2",
            stats,
            "python_intelligence",
    ...
    if core_dir.exists():
        count = copy_directory(
            core_dir, py_dir / "core", stats, "python_intelligence", extensions=[".py", ".json"]
        )
        ...
        for project_dir in src_sessions.iterdir():
            if project_dir.is_dir():
                ...
                if conv_dir.exists():
                    ...
                    count = copy_directory(conv_dir, dst, stats, "sessions", extensions=[".json"])
                    print(f"  [+] {project_dir.name}/conversations: {count} files")

    src_backups = BASE_DIR / "backups"
    if src_backups.exists():
        count = copy_directory(
            src_backups, backups_dir, stats, "backups", extensions=[".zip", ".json", ".tar.gz"]
        )
    ...
    print("\nBundle Contents:")
    print("  - Backend System (TypeScript)")
    print("  - Frontend Dashboard (React)")
    print("  - Database (SQLite + WAL)")
    print("  - Python Intelligence")
    print("  - Knowledge Base")
    print("  - Sessions & Backups")
    print("  - Documentation")
