...
def test_aurora_response(input_data: Any, **kwargs) -> Any:
    """
    ...
    Examples:
        >>> test_aurora_response("test")
        'PROCESSED: test'
        >>> test_aurora_response(42)
        'PROCESSED: 42'
    """
    # Validate input
...
def test_aurora_response_batch(items: list[Any], **kwargs) -> list[Any]:
    ...
    Args:
        ...
        **kwargs: Options to pass to test_aurora_response

    Returns:
        List of processed results

    Examples:
        >>> test_aurora_response_batch(["a", "b", "c"])
...
def test_aurora_response_async(input_data: Any, callback: Callable | None = None) -> Any:
    ...
    Returns:
        Processed result
    """
    result = test_aurora_response(input_data)
...
class TestTestAuroraResponse:
    ...
    def test_string_input(self):
        ...
        result = test_aurora_response("hello")
        assert result == "PROCESSED: hello"

    def test_numeric_input(self):
        """Test with numeric input"""
        assert test_aurora_response(42) == "PROCESSED: 42"
        assert test_aurora_response(3.14) == "PROCESSED: 3.14"

    def test_list_input(self):
        """Test with list input"""
        result = test_aurora_response([1, 2, 3])
        assert result == ["PROCESSED: 1", "PROCESSED: 2", "PROCESSED: 3"]

    def test_dict_input(self):
        """Test with dictionary input"""
        result = test_aurora_response({"a": 1, "b": 2})
    ...
    def test_with_options(self):
        ...
        result = test_aurora_response("test", uppercase=True)
        assert "PROCESSED" in result

        result = test_aurora_response("test", reverse=True)
        assert result[::-1] == "PROCESSED: test"

    def test_batch_processing(self):
        """Test batch processing"""
        items = ["a", "b", "c"]
        results = test_aurora_response_batch(items)
...
class TestTestAuroraResponseIntegration:
    """Integration tests"""
    ...
    def test_end_to_end_workflow(self):
        ...
        # Step 2: Process
        result = test_aurora_response(data)
        assert "PROCESSED" in result
...
if __name__ == "__main__":
    ...
    for example in examples:
        try:
            demo_result = test_aurora_response(example)
    ...
    print("\nWith options:")
    print(f"{func_name}('test', uppercase=True) = {repr(test_aurora_response('test', uppercase=True))}")
    print(f"{func_name}('test', reverse=True)   = {repr(test_aurora_response('test', reverse=True))}")
    ...
    try:
        ...
        test_basic = TestTestAuroraResponse()
        test_integration = TestTestAuroraResponseIntegration()
    ...
    # Run benchmark
    benchmark_test_aurora_response(100)
