...
def _git_commit_push(msg:str)->bool:
    if not _git_available(): return False
    _run("git add -A")
...
def compile_from_nl(prompt: str)->BridgeResult:
    ...
    ts = res.manifest["ts"]
    return BridgeResult(ok=ok, message="compiled", run_dir=str(res.run_dir),
...
def compile_from_spec(spec_path: str)->BridgeResult:
    sp = Path(spec_path)
    if not sp.exists(): return BridgeResult(False, f"spec not found: {spec_path}")
    ...
    return BridgeResult(ok=ok, message="spec-compiled" if ok else "spec-failed",
                        run_dir=latest, files=None, zip_rel=None, logs=[out,err])
