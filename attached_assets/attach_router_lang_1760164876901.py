    ...
    def chat():
        ...
        if intent.kind == "web_app":
            if lang.lang == "python":
                ...
                return jsonify({"ok": True, "kind": "web_app", "lang":"python", "file":"app.py", "reason": lang.reason, "hint":"PORT=8000 python app.py"})
            if lang.lang == "go":
                pkg = render_go_service(intent.name)
                for fname, src in pkg["files"].items():
                    Path(fname).parent.mkdir(parents=True, exist_ok=True)
                    Path(fname).write_text(src, encoding="utf-8")
                return jsonify({"ok": True, "kind": "web_app", "lang":"go", "files": list(pkg["files"].keys()), "reason": lang.reason, "hint":"PORT=8080 go run ."})
            if lang.lang == "csharp":
                ...
                return jsonify({"ok": True, "kind": "web_app", "lang":"csharp", "folder": str(folder), "files": list(pkg["files"].keys()), "reason": lang.reason, "hint":"PORT=5080 dotnet run"})
            # fallback
            code = render_app(title=intent.name.replace('_',' ').title(), subtitle=intent.brief)
            Path("app.py").write_text(code, encoding="utf-8")
            return jsonify({"ok": True, "kind": "web_app", "lang":"python", "file":"app.py", "reason":"fallback", "hint":"PORT=8000 python app.py"})
