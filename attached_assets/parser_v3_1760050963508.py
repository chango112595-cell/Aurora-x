...
def parse_functions(md: str) -> list[FunctionSpec]:
    ...
    for ch in chunks:
        ...
        out.append(FunctionSpec(name=name, signature=signature, description=desc,
                                preconditions=pre, postconditions=post, examples=exs))
    return out
...
def parse_v3(md: str) -> RichSpecV3:
    ...
    if '### Function' in md:
        ...
        funcs = [FunctionSpec(name=signature.split('(')[0].split()[-1],
                              signature=signature, description=desc,
                              postconditions=post, examples=exs)]
