...
def iter_multi_items(
    mapping: MultiDict[K, V] | cabc.Mapping[K, V | list[V] | tuple[V, ...] | set[V]] | cabc.Iterable[tuple[K, V]],
...
class MultiDict(TypeConversionDict[K, V]):
    ...
    def __init__(
        self,
        mapping: (
            MultiDict[K, V] | cabc.Mapping[K, V | list[V] | tuple[V, ...] | set[V]] | cabc.Iterable[tuple[K, V]] | None
        ) = None,
    ) -> None:
        if mapping is None:
            super().__init__()
...
class _OrderedMultiDict(MultiDict[K, V]):
    ...
    def __init__(
        ...
        warnings.warn(
            "'OrderedMultiDict' is deprecated and will be removed in Werkzeug" " 3.2. Use 'MultiDict' instead.",
            DeprecationWarning,
            stacklevel=2,
        )
        super().__init__()
...
class CombinedMultiDict(ImmutableMultiDictMixin[K, V], MultiDict[K, V]):  # type: ignore[misc]
    ...
    def items(self, multi: bool = False) -> cabc.Iterable[tuple[K, V]] | cabc.Iterable[tuple[K, list[V]]]:
        found = set()
        for d in self.dicts:
...
class _ImmutableOrderedMultiDict(ImmutableMultiDictMixin[K, V], _OrderedMultiDict[K, V]):  # type: ignore[misc]
