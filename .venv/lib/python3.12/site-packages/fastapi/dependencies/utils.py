...
def get_parameterless_sub_dependant(*, depends: params.Depends, path: str) -> Dependant:
    ...
    return get_dependant(path=path, call=depends.dependency, security_scopes=use_security_scopes)
...
def get_dependant(
    *,
    ...
    if isinstance(call, SecurityBase):
        ...
        security_requirement = SecurityRequirement(security_scheme=call, scopes=use_scopes)
    ...
    for param_name, param in signature_params.items():
        ...
        if isinstance(param_details.field.field_info, (params.Body, temp_pydantic_v1_params.Body)):
...
def analyze_param(
    *,
    ...
    if get_origin(use_annotation) is Annotated:
        ...
        fastapi_annotations = [
            arg for arg in annotated_args[1:] if isinstance(arg, (FieldInfo, may_v1.FieldInfo, params.Depends))
        ]
        ...
        if fastapi_specific_annotations:
            fastapi_annotation: FieldInfo | may_v1.FieldInfo | params.Depends | None = fastapi_specific_annotations[-1]
        else:
    ...
    # Handle default assignations, neither field_info nor depends was not found in Annotated nor default value
        ...
        if is_path_param:
            ...
            if annotation_is_pydantic_v1(use_annotation):
                field_info = temp_pydantic_v1_params.Body(annotation=use_annotation, default=default_value)
            else:
    ...
    if field_info is not None:
        ...
        if is_path_param:
            assert isinstance(field_info, (params.Path, temp_pydantic_v1_params.Path)), (
...
def _get_multidict_value(field: ModelField, values: Mapping[str, Any], alias: str | None = None) -> Any:
...
async def request_body_to_args(
    ...
    if single_not_embedded_field and _is_model_class(first_field.type_) and isinstance(received_body, FormData):
        fields_to_extract = get_cached_model_fields(first_field.type_)
...
def get_body_field(*, flat_dependant: Dependant, name: str, embed_body_fields: bool) -> ModelField | None:
    ...
    elif any(isinstance(f.field_info, temp_pydantic_v1_params.File) for f in flat_dependant.body_params):
        BodyFieldInfo: type[temp_pydantic_v1_params.Body] = temp_pydantic_v1_params.File  # type: ignore[no-redef]
    elif any(isinstance(f.field_info, params.Form) for f in flat_dependant.body_params):
        BodyFieldInfo = params.Form
    elif any(isinstance(f.field_info, temp_pydantic_v1_params.Form) for f in flat_dependant.body_params):
