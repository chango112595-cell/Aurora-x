...
SERVICES = [
    {
        "name": "Memory Fabric V2",
        "cmd": ["python3", "aurora_memory_fabric_v2/service.py"],
        "port": 5004,
    },
    {"name": "Memory Bridge", "cmd": ["python3", "server/memory-bridge.py"], "port": 5003},
    {
        "name": "Luminar Nexus V2",
        "cmd": ["python3", "tools/luminar_nexus_v2.py", "serve"],
        "port": 8000,
    },
...
def get_service_status():
    ...
    for name, p, port in processes:
        ...
        status.append({"name": name, "alive": alive, "port": port, "port_responding": port_open})
    return status
