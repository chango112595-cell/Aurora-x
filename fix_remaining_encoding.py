...
content = re.sub(r"open\((['\"][^'\"]+['\"]),\s*['\"]r['\"]\)",
                 r"open(\1, 'r', encoding='utf-8')", content)
content = re.sub(r"open\((['\"][^'\"]+['\"]),\s*['\"]w['\"]\)",
                 r"open(\1, 'w', encoding='utf-8')", content)
    ...
    for i, line in enumerate(lines):
        if 'def transform_output' in line and 'output_format' in line:
            ...
            if i+2 < len(lines) and 'output_format' not in lines[i+2]:
                lines.insert(
                    i+2, '    _ = output_format  # Format specification\n')
    with open(filepath, 'w', encoding='utf-8') as f:
        f.writelines(lines)
