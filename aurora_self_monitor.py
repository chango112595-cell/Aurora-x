...
✅ STATUS: MONITORING ACTIVE - ALL SYSTEMS OPERATIONAL
    ...
    def continuous_monitor(self, duration_seconds: int = 60):
        ...
        while time.time() < end_time:
            ...
            print(f"[{health['timestamp']}] Status: {health['status']} | CPU: {health['cpu_percent']:.1f}% | Memory: {health['memory_percent']:.1f}%")
    ...
    def get_summary(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'average_cpu': sum(h['cpu_percent'] for h in self.health_log) / len(self.health_log) if self.health_log else 0,
            'average_memory': sum(h['memory_percent'] for h in self.health_log) / len(self.health_log) if self.health_log else 0
        }
