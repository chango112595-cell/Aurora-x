...
class AuroraSystemUpdater:
    ...
    def update_frontend_components(self, counts: Dict[str, int]) -> None:
        ...
        for file_str in frontend_files:
            ...
            content = re.sub(r'<span className="text-purple-400 font-mono text-lg">(\d+)</span>',
                             lambda m: f'<span className="text-purple-400 font-mono text-lg">{tier_count}</span>'
                             if 'Knowledge Tiers' in original[max(0, m.start()-200):m.start()]
                             else m.group(0), content)
            content = re.sub(r'<span className="text-pink-400 font-mono text-lg">(\d+)</span>',
                             lambda m: f'<span className="text-pink-400 font-mono text-lg">{total}</span>'
    ...
    def update_documentation(self, counts: Dict[str, int]) -> None:
        ...
        if integration_file.exists():
            ...
            content = re.sub(
                r"'total_capabilities': \d+",
                f"'total_capabilities': {counts['total_capabilities']}",
                content
            )
            content = re.sub(
                r"'knowledge_tiers': \d+",
                f"'knowledge_tiers': {counts['tier_count']}",
                content
            )
            ...
            if content != original:
                ...
                print(f"  ✅ aurora_autonomous_integration.py")
    ...
    def run(self) -> bool:
        ...
        print(f"  • All frontend components synchronized")
        print(f"  • All backend files synchronized")
