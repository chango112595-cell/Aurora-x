...
def test_execution():
    ...
    try:
        result = subprocess.run(["python", "test_cli_hash.py", "--help"], capture_output=True, text=True, timeout=5)
...
"""
        ...
        result = subprocess.run(["python", "run_factorial_test.py"], capture_output=True, text=True, timeout=5)
        if result.returncode == 0:
