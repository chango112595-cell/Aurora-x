...
def test_api_endpoints():
    ...
    response = requests.post(f"{BASE_URL}/api/solve", json={"problem": "differentiate 3x^2 + 2x + 5"})
    print("   Request: differentiate 3x^2 + 2x + 5")
    print(f"   Response: {json.dumps(response.json(), indent=2)}")

    # Test math evaluation
    print("\n2. Testing Math Evaluation:")
    response = requests.post(f"{BASE_URL}/api/solve", json={"problem": "2 + 3 * 4"})
    print("   Request: 2 + 3 * 4")
    print(f"   Response: {json.dumps(response.json(), indent=2)}")

    # Test physics orbital period
    print("\n3. Testing Physics Orbital Period:")
    response = requests.post(f"{BASE_URL}/api/solve", json={"prompt": "orbital period a=7e6 M=5.972e24"})
    print("   Request: orbital period a=7e6 M=5.972e24")
    print(f"   Response: {json.dumps(response.json(), indent=2)}")

    # Test explain endpoint
    print("\n4. Testing Explain Endpoint:")
    response = requests.post(f"{BASE_URL}/api/explain", json={"problem": "differentiate x^3"})
