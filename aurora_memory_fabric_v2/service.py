...
memory: AuroraMemoryFabric | None = None
    ...
    def do_GET(self):
        ...
        try:
            ...
            if path == "/status":
                ...
                self.send_json(
                    {
                        ...
                        "stats": {
                            ...
                            "totalMemories": stats["short_term_count"]
                            + stats["mid_term_count"]
                ...
                facts = mem.get_all_facts()
                self.send_json({"success": True, "facts": facts})

            elif path == "/context":
                context = mem.get_context_summary()
                self.send_json({"success": True, "context": context})

            elif path == "/integrity":
                hashes = mem.integrity_hash()
                self.send_json({"success": True, "integrity": hashes})
    ...
    def do_POST(self):
        ...
        try:
            ...
            if path == "/message":
                ...
                self.send_json({"success": True, "entry": self._entry_to_dict(entry)})
                ...
                self.send_json({"success": True, "key": key, "value": value, "category": category})
                ...
                self.send_json(
                    {"success": True, "results": [self._entry_to_dict(e) for e in results]}
                )
                ...
                self.send_json({"success": True, "conversationId": conv_id})

            elif path == "/backup":
                backup_path = mem.backup()
                self.send_json({"success": True, "backupPath": backup_path})
                ...
                self.send_json({"success": True, "key": key, "value": value})
