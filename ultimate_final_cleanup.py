...
content = content.replace(
    'for _service_name, service_config', 'for service_name, service_config')
...
for filepath in ['create_a_simple_hello_world.py', 'test.py', 'test_aurora_response.py', 'test_lib_generic.py']:
    ...
    for i, line in enumerate(lines):
        ...
        if re.search(r'\bif\s+callback\s*:', line) and 'callable' not in line and 'is not None' not in line:
            lines[i] = re.sub(r'if\s+callback\s*:',
                              'if callable(callback):', line)
        # Change "if not callback:" to "if not callable(callback):"
        if re.search(r'\bif\s+not\s+callback\s*:', line) and 'callable' not in line:
            lines[i] = re.sub(r'if\s+not\s+callback\s*:',
                              'if not callable(callback):', line)
...
for i, line in enumerate(lines):
    ...
    if i > 285 and i < 295:
        if ', format: str =' in line or ', format=' in line:
            lines[i] = line.replace(', format:', ', fmt:').replace(
                ', format=', ', fmt=')
...
with open('test_healthz.py', 'r', encoding='utf-8') as f:
    lines = f.readlines()
for i, line in enumerate(lines):
    if i in [18, 19, 70, 71]:
        if "open(" in line and "encoding=" not in line:
...
with open('test_t08_e2e.py', 'r', encoding='utf-8') as f:
    lines = f.readlines()
for i, line in enumerate(lines):
    if i in [91, 92]:
        if "open(" in line and "encoding=" not in line:
...
with open('test_t08_offline.py', 'r', encoding='utf-8') as f:
    lines = f.readlines()
for i, line in enumerate(lines):
    if i in [88, 89]:
        if "open(" in line and "encoding=" not in line:
...
with open('test_lib_generic.py', 'r', encoding='utf-8') as f:
    lines = f.readlines()
for i, line in enumerate(lines):
        ...
        if 'def transform_output' in line and 'output_format' in line:
            ...
            if j < len(lines) and '_ = output_format' not in lines[j]:
                ...
                lines.insert(j, ' ' * indent +
                             '_ = output_format  # Format specification\n')
    with open(filepath, 'w', encoding='utf-8') as f:
        f.writelines(lines)
