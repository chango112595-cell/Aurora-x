        ...
        while time.time() - start < self.timeout:
            try:
                ...
                self.lock_file.write(
                    json.dumps({"pid": os.getpid(), "acquired": datetime.now(UTC).isoformat()})
                )
...
class EtcdStore:
        ...
        if self.store_file.exists():
            try:
                with open(self.store_file) as f:
                    data = json.load(f)
                    self._local_cache = {
                        k: RegistryEntry(**v) if isinstance(v, dict) else v for k, v in data.items()
                    }
    ...
    def put(self, key: str, value: Any, ttl: int | None = None) -> RegistryEntry:
        ...
        with self._cache_lock:
            ...
            version = (
                (existing.version + 1) if existing and isinstance(existing, RegistryEntry) else 1
            )

            expires = None
            if ttl:
                expires = datetime.fromtimestamp(time.time() + ttl, tz=UTC).isoformat()
            ...
            entry = RegistryEntry(
                ...
                if existing and isinstance(existing, RegistryEntry)
                else now,
                modified=now,
                ttl=ttl,
                expires=expires,
    ...
    def register_module(self, module_id: str, module_data: dict) -> RegistryEntry:
        ...
        module_data["registered"] = datetime.now(UTC).isoformat()
