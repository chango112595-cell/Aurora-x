...
class PatternChecker:
    ...
    def __init__(self):
        self.compiled_banned = [
            (re.compile(p), n, s, c, m) for p, n, s, c, m in self.BANNED_PATTERNS
        ]
        self.compiled_warnings = [
            (re.compile(p), n, s, c, m) for p, n, s, c, m in self.WARNING_PATTERNS
        ]
...
class ASTAnalyzer:
    ...
    def _check_complexity(self, tree: ast.AST):
        ...
        for node in ast.walk(tree):
            if isinstance(
                node, (ast.If, ast.While, ast.For, ast.Try, ast.ExceptHandler, ast.With, ast.Assert)
            ):
                complexity += 1
...
class CodeInspector:
    ...
    def inspect(self, module_path: str) -> InspectionResult:
        ...
        try:
            ...
            return InspectionResult(path=str(path), safe=False, error=f"File not found: {path}")
        except Exception as e:
            return InspectionResult(path=str(path), safe=False, error=str(e))
