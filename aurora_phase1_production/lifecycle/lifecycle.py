...
class ModuleLoader:
    ...
    def load_module(self, module_path: str) -> Any | None:
    ...
    def get_entry_point(self, module: Any, entry_point: str) -> Callable | None:
...
class ModuleLifecycle:
    ...
    def init(self, config: dict = None, timeout: float = 30.0) -> LifecycleResult:
        ...
        if result.success:
            ...
            self.state.initialized_at = datetime.now(UTC).isoformat()
    ...
    def execute(self, payload: dict = None, timeout: float = 30.0) -> LifecycleResult:
        ...
        if result.success:
            ...
            self.state.last_executed = datetime.now(UTC).isoformat()
    ...
    def run_full_lifecycle(
        self, config: dict = None, payload: dict = None, timeout: float = 30.0
    ) -> dict:
...
class LifecycleRunner:
    ...
    def get_or_create_lifecycle(self, module_id: str, category: str) -> ModuleLifecycle:
        ...
        if key not in self.lifecycles:
            self.lifecycles[key] = ModuleLifecycle(
                module_id=module_id, category=category, base_dir=str(self.modules_dir)
            )
    ...
    def run_batch(self, modules: list[dict], workers: int = 4, timeout: float = 30.0) -> list[dict]:
        ...
        for path in search_dir.rglob("*_execute.py"):
            ...
            if len(parts) >= 2:
                ...
                if len(name_parts) >= 2:
                    cat = name_parts[0]
                    mod_id = name_parts[1]
                    modules.append({"id": mod_id, "category": cat, "path": str(path.parent)})
