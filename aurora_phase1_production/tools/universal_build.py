...
class UniversalBuilder:
    ...
    def build_modules(self, output_dir: str = None, count: int = 550,
                      create_zip: bool = True, with_registry: bool = True) -> Dict:
    ...
    def full_build(self, output_dir: str = None, gpu: bool = True,
                   module_count: int = 550) -> Dict:
        ...
        logger.info(f"Python: {checks['python']['version']} - {'OK' if checks['python']['ok'] else 'FAIL'}")
        logger.info(f"Node.js: {checks['node'].get('version', 'N/A')} - {'OK' if checks['node']['ok'] else 'Optional'}")
        logger.info(f"GPU: {checks['gpu']['gpu_count']} available - {'CUDA' if checks['gpu']['cuda_available'] else 'CPU'}")
...
def main():
    parser = argparse.ArgumentParser(
        description="Aurora-X Ultra Universal Build Script"
    )
    parser.add_argument(
        "--output", "-o",
        type=str,
        default="aurora_build",
    ...
    parser.add_argument(
        ...
        type=int,
        default=550,
        help="Number of modules to generate (default: 550)"
    )
    parser.add_argument(
        "--no-gpu",
        action="store_true",
