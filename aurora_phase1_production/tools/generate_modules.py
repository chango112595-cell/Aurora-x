...
def generate_module(module: dict, output_dir: Path, force: bool = False) -> dict:
    ...
    return {"module_id": module["id"], "category": module["category"], "files": files_generated}
...
def main():
    parser = argparse.ArgumentParser(description="Generate Aurora modules from manifest")
    parser.add_argument("--manifest", "-m", type=str, required=True, help="Path to manifest file")
    parser.add_argument(
        "--out",
        "-o",
        type=str,
