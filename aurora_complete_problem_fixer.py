...
class AuroraCompleteFixer:
    ...
    def fix_unused_imports(self, filepath: Path) -> bool:
        ...
        try:
            ...
            unused_imports = [
                i for i in issues
                if i.get('path') == str(filepath) and i.get('message-id') == 'W0611'
            ]
    ...
    def fix_subprocess_check(self, filepath: Path) -> bool:
        ...
        try:
            ...
            content = re.sub(
                r'subprocess\.run\((.*?)\)',
                lambda m: f'subprocess.run({m.group(1, check=False)}, check=False)'
                if 'check=' not in m.group(1) else m.group(0),
                content
            )
    ...
    def fix_redefined_outer_name(self, filepath: Path) -> bool:
        ...
        try:
            ...
            patterns = [
                (r'def \w+\([^)]*\bold\b[^)]*\):',
                 lambda m: m.group(0).replace('old', 'old_val')),
                (r'def \w+\([^)]*\bnew\b[^)]*\):',
                 lambda m: m.group(0).replace('new', 'new_val')),
                (r'def \w+\([^)]*\bcontent\b[^)]*\):',
                 lambda m: m.group(0).replace('content', 'content_val')),
                (r'def \w+\([^)]*\bf\b[^)]*\):',
                 lambda m: m.group(0).replace('', 'file_obj')),
                (r'def \w+\([^)]*\bprompt\b[^)]*\):',
                 lambda m: m.group(0).replace('prompt', 'prompt_text')),
                (r'def \w+\([^)]*\bresult\b[^)]*\):',
                 lambda m: m.group(0).replace('result', 'result_val')),
                (r'def \w+\([^)]*\bapp\b[^)]*\):',
                 lambda m: m.group(0).replace('app', 'app_instance')),
            ]
        ...
        for filepath in self.target_files:
            ...
            if fixes > 0:
                ...
                if not self.validate_syntax(filepath):
                    print(
                        f"   ⚠️  WARNING: {filepath.name} may have syntax issues")
