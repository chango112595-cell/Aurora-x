    ...
    def run_pylint_analysis(self):
        ...
        try:
            ...
            if result.stdout:
                ...
                print(
                    f"   Found {total} issues across {len(self.errors_by_type)} categories")
                            ...
                            if 'pylint: disable' not in line:
                                ...
                                if line_num > 1:
                                    lines.insert(line_num - 1,
                                                 ' ' * indent + '# pylint: disable=import-outside-toplevel\n')
    ...
    def fix_singleton_comparison(self):
        ...
        for filepath in self.all_python_files:
            try:
                ...
                content = re.sub(
                    r'(\w+\([^)]*\))\s*==\s*True\b', r'\1', content)

                # Fix == False (use not)
                content = re.sub(
                    r'(\w+\([^)]*\))\s*==\s*False\b', r'not \1', content)
