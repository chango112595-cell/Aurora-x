...
export async function getChatResponse(
  ...
  try {
    ...
    // First, try Enhanced Aurora Core Intelligence for actual thinking
    try {
...
`]);
      ...
      if (exitCode === 0 && output.trim()) {
        console.log('[Aurora Chat] ✨ Enhanced Aurora Core response generated');
        return {
          ok: true,
          response: output.trim()
        };
      } else if (error) {
        console.warn('[Aurora Chat] Enhanced Core error:', error);
      }
    } catch (coreError) {
      console.warn('[Aurora Chat] Enhanced Core unavailable:', coreError);
    }
    ...
    if (isSystemCommand) {
      try {
        ...
        if (v2Response.ok) {
          ...
          return {
            ok: true,
            response: v2Data.response || v2Data.message || 'Command processed.'
          };
        }
      } catch (v2Error) {
        console.warn('[Aurora Chat] Luminar V2 unavailable, using fallback');
      }
    }
