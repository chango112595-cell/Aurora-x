...
def register_node(node_id: str, meta: dict[str, Any] | None = None) -> dict[str, Any]:
    nodes = _load_nodes()
    for node in nodes:
...
def heartbeat(node_id: str) -> dict[str, Any] | None:
    nodes = _load_nodes()
    for node in nodes:
        if node.node_id == node_id:
            node.last_seen = time.time()
...
def execute(command: str, params: dict = None):
    ...
    if command == "register_node":
        return {
            "status": "ok",
            "node": register_node(params.get("node_id", "unknown"), params.get("meta")),
            "ts": time.time(),
        }
    ...
    if command == "prune_stale":
        return {
            "status": "ok",
            "nodes": prune_stale(float(params.get("max_age", 300))),
            "ts": time.time(),
        }
    if command == "topology":
