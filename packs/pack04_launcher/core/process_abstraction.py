...
class PackProcess:
    def __init__(self, pack_id: str, workdir: str | None = None):
        self.pack_id = pack_id
        self.workdir = Path(workdir) if workdir else (ROOT / "data" / "vfs" / pack_id)
        self.workdir.mkdir(parents=True, exist_ok=True)

    def run(self, cmd: str, timeout: int | None = 30, capture=True):
    ...
    def run_background(self, cmd: str):
        args = shlex.split(cmd)
        p = subprocess.Popen(
            args, cwd=str(self.workdir), stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL
        )
        return p.pid
