...
def record_observation(
    category: str, value: float, tags: dict[str, Any] | None = None
) -> dict[str, Any]:
...
def detect_anomalies(category: str, threshold: float = 2.0) -> list[dict[str, Any]]:
    ...
    anomalies = [
        obs for obs in observations if abs(float(obs.get("value", 0)) - mean) / stdev >= threshold
    ]
    return anomalies
...
def execute(command: str, params: dict = None):
    ...
    if command == "aggregate":
        return {
            "status": "ok",
            "summary": aggregate(params.get("category", "signal")),
            "ts": time.time(),
        }
    if command == "anomalies":
        return {
            ...
            "anomalies": detect_anomalies(
                params.get("category", "signal"), float(params.get("threshold", 2.0))
            ),
            "ts": time.time(),
        }
