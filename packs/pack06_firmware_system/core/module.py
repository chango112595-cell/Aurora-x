...
def _compute_checksum(payload: str | None) -> str:
    if payload is None:
        payload = ""
...
def register_firmware(
    name: str, version: str, image: str | None = None, metadata: dict[str, Any] | None = None
):
    ...
    _record_update(
        {"event": "registered", "firmware_id": firmware_id, "name": name, "version": version}
    )
...
def latest_firmware(name: str | None = None) -> dict[str, Any] | None:
    records = _load_registry()
    if name:
