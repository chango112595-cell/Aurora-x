...
def test_module_manifest_exists():
    ...
    assert len(data["modules"]) == 550, f"Manifest should list 550 modules, found {len(data['modules'])}"
...
def test_sample_modules_structure():
    ...
    for module_id in sample_ids:
        ...
        for method in required_methods:
            ...
            assert callable(getattr(instance, method)), f"{method} is not callable in module {module_id}"
...
def test_required_functions_exist():
    ...
    assert len(errors) == 0, f"Module integrity errors:\n" + "\n".join(errors[:20])
