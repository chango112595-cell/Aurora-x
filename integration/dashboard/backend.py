...
def list_staged():
    staged = (
        [d.name for d in (UPDATER_DIR / ".aurora_updates" / "staging").glob("*") if d.is_dir()]
        if (UPDATER_DIR / ".aurora_updates" / "staging").exists()
        else []
    )
...
def stage_file(filename: str = Form(...)):
    ...
    from integration.updater.updater_service import stage_artifact
...
def suggestions_list():
    ...
    roots = [
        ...
        Path("automotive/suggestions"),
        Path("aviation/suggestions"),
    ]
    out = {}
    for r in roots:
        if r.exists():
...
def marketplace_install(pkg_name: str):
    ...
    with open(AUDIT_LOG, "a") as fh:
        fh.write(f"{time.time()} INSTALL {pkg_name}\n")
    return {"ok": True}
