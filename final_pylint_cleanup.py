...
if os.path.exists('create_a_simple_hello_world.py'):
    ...
    content = content.replace('transform_output(result, format="json")',
                              'transform_output(result, output_format="json")')
    content = content.replace('transform_output(result, format="xml")',
                              'transform_output(result, output_format="xml")')

    # Fix undefined variable 'result'
    content = content.replace('print(f"{func_name}({repr(example):20s}) = {repr(result)}")',
...
if os.path.exists('diagnostic_server.py'):
    ...
    content = content.replace('elif format == "text":',
...
for filepath in ['generated_timer_app.py', 'generated_web_app.py']:
    if os.path.exists(filepath):
        ...
        content = content.replace(
            'def setup_routes(app):', 'def setup_routes(flask_app):')
                ...
                if 'n =' in lines[i] and 'range(' in ''.join(lines[max(0, i-2):min(len(lines), i+3)]):
                    ...
                    for j in range(i+1, min(len(lines), i+15)):
                        lines[j] = lines[j].replace(
                            'range(n)', 'range(num_iterations)')
                        lines[j] = lines[j].replace(
                            'for _ in range(n)', 'for _ in range(num_iterations)')
                    break
