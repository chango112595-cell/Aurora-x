...
def test_chat_endpoint():
    ...
    for i, test in enumerate(test_cases, 1):
        ...
        if "expected_file" in test:
            assert data.get("file") == test["expected_file"], f"Expected file={test['expected_file']}, got {data.get('file')}"
            # Check if file was created for web_app
            if test["expected_kind"] == "web_app":
                assert os.path.exists("app.py"), "app.py should be created"
                print(f"  ✅ File app.py created successfully")
        
        if "expected_note" in test:
            assert data.get("note") == test["expected_note"], f"Expected note={test['expected_note']}, got {data.get('note')}"
    ...
    data = response.json()
    print(f"  Response: {json.dumps(data, indent=2)}")
    assert data["ok"] == False, "Empty prompt should return ok=False"
