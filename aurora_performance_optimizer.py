...
class AuroraPredictor:
    ...
    def generate_early_warning(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'estimated_prevention_time': sum(
                30 if p['severity'] == 'HIGH' else 15
...
def main():
    ...
    for pred in predictions:
        print(
            f"  • {pred['issue_type']} [{pred['severity']}] - {pred['probability']*100:.0f}% probability")
    ...
    print(
        f"  • Prevention time saved: {warning['estimated_prevention_time']} minutes")
    ...
    if bottlenecks:
        ...
        for bn in bottlenecks:
            print(
                f"  • {bn['operation']}: {bn['time']*1000:.2f}ms [{bn['severity']}]")
            print(
                f"    Optimization potential: {bn['optimization_potential']}")
    ...
    print(f"  • Expected speed increase: 50-70%")
    print("="*60)
