...
class MultiIntentParser:
    ...
    def _detect_project_type(self, prompt: str) -> ProjectType:
        ...
        for ptype, patterns in self.PATTERNS.items():
            score = sum(1 for pattern in patterns if re.search(
                pattern, prompt, re.I))
            scores[ptype] = score
...
"""
    ...
    def _sql_schema_code(self, intent: ParsedIntent) -> str:
        ...
        for model in models:
            table_name = model.lower() if model.endswith(
                "s") else f"{model.lower()}s"
            schema += f"""
...
async def synthesize_universal(prompt: str, run_id: str = None, context: dict = None) -> dict:
    ...
    try:
        ...
        run_dir = Path("runs") / \
            (run_id or f"run-{time.strftime('%Y%m%d-%H%M%S')}")
...
if __name__ == "__main__":
    ...
    if result.get("validation"):
        if result["validation"]["is_valid"]:
            ...
            print(
                f"[WARN] {len(result['validation']['issues'])} validation issues found")
