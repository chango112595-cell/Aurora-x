...
def start_learning(sleep_seconds: int = 300, max_iters: int = 50, beam: int = 20):
    ...
    if daemon_stats["status"] == "running":
        return JSONResponse(
            content={"ok": False, "error": "Self-learning daemon already running"},
            status_code=400
        )
...
def stop_learning():
    ...
    if daemon_stats["status"] != "running":
        return JSONResponse(
            content={"ok": False, "error": "Self-learning daemon not running"},
            status_code=400
        )
    ...
    return JSONResponse(content={
        "ok": True,
        "message": "Self-learning daemon stopped"
    })
