...
def parse_english(text: str) -> NLParseResult:
        ...
        return NLParseResult(
            {
                ...
                "examples": [{"s": "racecar", "out": True}, {"s": "hello", "out": False}, {"s": "a", "out": True}],
            }
        )
    if "fibonacci" in t:
        return NLParseResult(
        ...
        return NLParseResult(
            {
                ...
                "examples": [{"nums": [3, 1, 2], "out": [1, 2, 3]}, {"nums": [5, -1, 0], "out": [-1, 0, 5]}],
            }
        )
        ...
        return NLParseResult(
            {
                ...
                "examples": [{"s": "hello", "out": 2}, {"s": "xyz", "out": 0}, {"s": "aeiou", "out": 5}],
            }
        )
    ...
    if any(
        word in t for word in ["calculate", "compute", "count", "number", "sum", "total", "how many", "value", "result"]
    ):
        return_type = "int"
        ...
        word in t
        for word in ["generate", "create", "make", "produce", "write", "compose", "haiku", "poem", "story", "text"]
    ):
        return_type = "str"
    ...
    needs_input = any(
        word in t for word in ["given", "from", "input", "parameter", "argument", "process", "convert", "transform"]
    )
    ...
    return NLParseResult({"name": func_name, "signature": signature, "description": description, "examples": examples})
