...
def synth(spec: Path):
    ...
    try:
        ...
        result = subprocess.run(
            ["python", "tools/spec_compile_v3.py", str(spec)], env=os.environ.copy(), capture_output=True, text=True
        )
        ...
        if result.returncode == 0:
            ...
            if DISCORD.exists():
                try:
                    subprocess.run(["python", str(DISCORD), "success", f"🔄 Auto-synth: {spec.name}"], check=False)
                except Exception:
                    pass
        else:
            ...
            if DISCORD.exists():
                try:
                    subprocess.run(["python", str(DISCORD), "error", f"❌ Auto-synth failed: {spec.name}"], check=False)
                except Exception:
                    pass
