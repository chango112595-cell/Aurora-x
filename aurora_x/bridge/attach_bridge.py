...
def attach_bridge(app: FastAPI):
    ...
    def bridge_comparison_commits():
        ...
        if code == 0:
            ...
            return JSONResponse(
                {"ok": False, "err": err or "Failed to get commits"},
                status_code=500
            )
    ...
    def bridge_comparison_diff(commit1: str | None = None, commit2: str | None = None):
        ...
        if code == 0:
            ...
            return JSONResponse(
                {"ok": False, "err": err or "Failed to get diff"},
                status_code=500
            )
    ...
    def get_commits():
        ...
        try:
            ...
            result = subprocess.check_output([
                "git", "log", "--oneline", "-20", "--pretty=format:%H|%s"
            ], cwd=".", text=True)
    ...
    def get_branches():
        ...
        try:
            ...
            current_branch = subprocess.check_output(["git", "rev-parse", "--abbrev-ref", "HEAD"], cwd=".", text=True).strip()
            ...
            for branch in branches:
                try:
                    ...
                    if branch == current_branch:
                        commit_count = int(subprocess.check_output(["git", "rev-list", "--count", "HEAD"], cwd=".", text=True).strip())
                    else:
                        try:
                            commit_count = int(subprocess.check_output(["git", "rev-list", "--count", f"origin/{branch}"], cwd=".", text=True).strip())
                        except:
                            commit_count = 0
                    ...
                    try:
                        if branch == current_branch:
                            last_commit = subprocess.check_output(["git", "log", "-1", "--format=%h", "HEAD"], cwd=".", text=True).strip()
                            last_commit_msg = subprocess.check_output(["git", "log", "-1", "--format=%s", "HEAD"], cwd=".", text=True).strip()
                        else:
                            last_commit = subprocess.check_output(["git", "log", "-1", "--format=%h", f"origin/{branch}"], cwd=".", text=True).strip()
                            last_commit_msg = subprocess.check_output(["git", "log", "-1", "--format=%s", f"origin/{branch}"], cwd=".", text=True).strip()
                    except:
                        last_commit = "unknown"
                    ...
                    try:
                        if branch != 'main':
                            ...
                            try:
                                if branch == current_branch:
                                    commits_output = subprocess.check_output(["git", "log", "--oneline", f"main..HEAD"], cwd=".", text=True)
                                else:
                                    commits_output = subprocess.check_output(["git", "log", "--oneline", f"main..origin/{branch}"], cwd=".", text=True)
                                ...
                                ui_keywords = ['dashboard', 'ui', 'interface', 'visual', 'frontend', 'react', 'component']
                                ...
                                ui_count = sum(1 for commit in commit_lines if any(kw in commit.lower() for kw in ui_keywords))
                                api_count = sum(1 for commit in commit_lines if any(kw in commit.lower() for kw in api_keywords))
                                ai_count = sum(1 for commit in commit_lines if any(kw in commit.lower() for kw in ai_keywords))
                                config_count = sum(1 for commit in commit_lines if any(kw in commit.lower() for kw in config_keywords))
                            ...
                            try:
                                if branch == current_branch:
                                    diff_output = subprocess.check_output(["git", "diff", "--stat", "main", "HEAD"], cwd=".", text=True)
                                else:
                                    diff_output = subprocess.check_output(["git", "diff", "--stat", "main", f"origin/{branch}"], cwd=".", text=True)
                                ...
                                if lines and lines[-1].strip():
                                    ...
                                    summary = lines[-1]
                                    if 'file' in summary:
                                        file_changes = int(re.search(r'(\d+) files? changed', summary).group(1)) if re.search(r'(\d+) files? changed', summary) else 0
                                    if 'insertion' in summary:
                                        lines_added = int(re.search(r'(\d+) insertion', summary).group(1)) if re.search(r'(\d+) insertion', summary) else 0
                                    if 'deletion' in summary:
                                        lines_deleted = int(re.search(r'(\d+) deletion', summary).group(1)) if re.search(r'(\d+) deletion', summary) else 0
                            except:
                                pass
                            ...
                            if 'copilot' in branch_lower or ai_count > 0:
                                ...
                                improvement_score = 8
                                unique_features = ["AI-powered code generation", "Intelligent completions", "Enhanced developer experience"]
                                ...
                                improvement_score = 7
                                unique_features = ["Enhanced user interface", "Better visualization", "Improved user experience"]
                            elif 'config' in branch_lower or 'setup' in branch_lower or config_count > 0:
                                feature_category = "Configuration & Setup"
                                improvement_score = 6
                                unique_features = ["Configuration improvements", "Setup enhancements", "Build system updates"]
                                if 'implement-configuration-updates' in branch_lower:
                                    unique_features = ["Comprehensive build guide", "Improved .gitignore", "Build artifact management"]
                                ...
                                improvement_score = 7
                                unique_features = ["New API endpoints", "Enhanced backend services", "Improved data handling"]
                            elif lines_added > 100:
                                feature_category = "Major Feature Addition"
                                improvement_score = 8
                                unique_features = ["Significant code additions", "New functionality", "Feature enhancement"]
                            elif commit_count > 1200:  # Current branch has more commits
                                feature_category = "Active Development"
                                improvement_score = 9
                                unique_features = ["Latest development", "Most recent features", "Current working branch"]
