...
class SelfLearningDiagnostics:
    ...
    def _check_code_quality(self, repo_path: Path) -> float:
        ...
        try:
            ...
            req_files = list(repo_path.glob("requirements.txt")) + list(
                repo_path.glob("package.json")
            )
            if req_files:
                score += 10
    ...
    def get_learning_progress(self) -> dict:
        ...
        successful_runs = sum(
            1 for s in self.quality_scores if s.get("success", False) and s["score"] > 0
        )
    ...
    def load_diagnostics(self) -> dict | None:
