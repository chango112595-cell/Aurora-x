    ...
    def execute(self, func: Callable, *args, **kwargs) -> SandboxResult:
        ...
        try:
            ...
            return SandboxResult(
                success=True, output=result, execution_time_ms=(time.time() - start_time) * 1000
            )
            ...
            return SandboxResult(success=False, output=None, error="Memory limit exceeded")
    ...
    def execute(self, module_path: str, payload: dict[str, Any]) -> SandboxResult:
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            cmd = [
                ...
                f"{tmpdir}:/data:ro",
                "-v",
                f"{module_path}:/module:ro",
            ]

            if not self.config.network_enabled:
                cmd.extend(["--network", "none"])

            cmd.extend(["python:3.11-slim", "python", "/module", "--payload", "/data/payload.json"])

            try:
                result = subprocess.run(
                    cmd, capture_output=True, timeout=self.config.timeout_seconds, text=True
                )
                ...
                return SandboxResult(success=False, output=None, error=str(e))
...
class SandboxRunner:
    ...
    def get_sandbox(self, sandbox_type: SandboxType, config: SandboxConfig | None = None):
    ...
    def run_module(
        ...
        if sandbox_type == SandboxType.CONTAINER:
            return self._container_sandbox.execute(
                module_path, {"payload": payload, "context": context or {}}
            )
        else:
            return self._run_with_cgroup(module_path, payload, context, cfg)

    def _run_with_cgroup(
        self, module_path: str, payload: Any, context: dict[str, Any] | None, config: SandboxConfig
    ) -> SandboxResult:
    ...
    def run_batch(
        self, modules: list[dict[str, Any]], max_parallel: int = 4
        ...
        with ProcessPoolExecutor(max_workers=max_parallel) as executor:
            ...
            for future in futures:
                ...
                try:
                    ...
                    results[module_id] = SandboxResult(success=False, output=None, error=str(e))
