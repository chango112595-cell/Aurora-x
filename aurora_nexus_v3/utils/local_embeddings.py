...
class LocalEmbeddingStore:
    ...
    def __init__(self, db_path: str | Path = "data/embeddings.db"):
        """
    ...
    def count(self, category: str | None = None) -> int:
        ...
        with sqlite3.connect(self.db_path) as conn:
            if category:
                cursor = conn.execute(
                    "SELECT COUNT(*) FROM documents WHERE category = ?", (category,)
                )
            else:
...
class JSONEmbeddingStore:
    ...
    def __init__(self, json_path: str | Path = "data/embeddings.json"):
        """
    ...
    def search(
        self, query: str, top_k: int = 5, category: str | None = None, min_score: float = 0.0
    ) -> list[SearchResult]:
        """Perform similarity search."""
        query_embedding = generate_embedding(query)
        results: list[tuple[float, SearchResult]] = []
...
def get_rag_context(
    store: LocalEmbeddingStore | JSONEmbeddingStore,
    query: str,
    top_k: int = 3,
    min_score: float = 0.1,
) -> str:
...
_default_store: LocalEmbeddingStore | None = None
