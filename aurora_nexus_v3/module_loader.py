...
class SandboxedModule:
    ...
    def __init__(
        self, module_id: str, category: str, paths: dict[str, str], sandbox_type: str = "vm"
    ):
    ...
    def execute(
        self, payload: Any = None, context: dict[str, Any] | None = None, timeout: float = 30.0
    ) -> dict[str, Any]:
        """Run execution."""
        if not self._loaded:
            self.load()
...
class ModuleLoader:
    ...
    def load_registry(self) -> bool:
        ...
        self.registry = {m["id"]: m for m in data.get("modules", [])}
        logger.info(f"Loaded registry with {len(self.registry)} modules")
        return True

    def get_module(self, module_id: str) -> SandboxedModule | None:
    ...
    def get_modules_by_category(self, category: str) -> list[str]:
        ...
        return [mid for mid, info in self.registry.items() if info.get("category") == category]

    def get_modules_by_tag(self, tag: str) -> list[str]:
        """Get all module IDs with a specific tag."""
        return [mid for mid, info in self.registry.items() if tag in info.get("tags", [])]
    ...
    def initialize_all(self, category: str | None = None) -> dict[str, Any]:
        """Initialize all modules or all in a category."""
        module_ids = (
            self.get_modules_by_category(category) if category else list(self.registry.keys())
        )
        results = {}
...
def init_module(module_id: str, config: dict[str, Any] | None = None) -> dict[str, Any]:
    """Convenience function to initialize a module."""
    return get_loader().initialize_module(module_id, config)


def run_module(
    module_id: str, payload: Any = None, context: dict[str, Any] | None = None
) -> dict[str, Any]:
