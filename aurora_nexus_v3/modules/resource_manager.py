...
class ResourceManager:
    ...
    async def initialize(self):
        self.logger.info(
            f"Resource manager initialized with budget: "
    ...
    async def allocate(
        ...
        with self._lock:
            ...
            if current_memory + memory_mb > self.total_budget.memory_mb:
                self.logger.warning(f"Memory allocation denied for {owner}: would exceed budget")
                return None

            if current_cpu + cpu_percent > self.total_budget.cpu_percent:
                self.logger.warning(f"CPU allocation denied for {owner}: would exceed budget")
                return None
    ...
    async def get_usage(self) -> dict[str, Any]:
        ...
        return {
            ...
            "memory_percent": (total_memory / self.total_budget.memory_mb * 100)
            if self.total_budget.memory_mb > 0
            else 0,
            "cpu_allocated_percent": total_cpu,
            "cpu_budget_percent": self.total_budget.cpu_percent,
            "cpu_percent": (total_cpu / self.total_budget.cpu_percent * 100)
            if self.total_budget.cpu_percent > 0
            else 0,
    ...
    async def set_budget(self, memory_mb: int, cpu_percent: float, max_allocations: int):
        self.total_budget = ResourceBudget(
            memory_mb=memory_mb, cpu_percent=cpu_percent, max_allocations=max_allocations
        )
        self.logger.info(
            f"Budget updated: {memory_mb}MB, {cpu_percent}% CPU, max {max_allocations} allocations"
        )
