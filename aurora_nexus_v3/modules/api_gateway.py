...
class APIGateway:
    ...
    def _register_core_endpoints(self):
        ...
        self.register("GET", "/api/resources", self._resources_handler, description="Resource usage")
    ...
    async def handle_request(
        ...
        for mw in self.middleware:
            try:
                result = await mw(method, path, body, headers) if asyncio.iscoroutinefunction(mw) else mw(method, path, body, headers)
    ...
    async def _check_auth(self, headers: Optional[Dict[str, str]]) -> bool:
        if not headers:
            return False
