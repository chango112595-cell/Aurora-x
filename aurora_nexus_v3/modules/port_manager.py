...
class PortManager:
    ...
    async def _find_free_port(self, pool: str, protocol: PortProtocol) -> int | None:
    ...
    async def get_allocation(self, port: int) -> dict[str, Any] | None:
        with self._lock:
            if port in self.allocations:
    ...
    async def scan_range(
        self, start: int, end: int, protocol: PortProtocol = PortProtocol.TCP
    ) -> list[int]:
        in_use = []
    ...
    async def get_stats(self) -> dict[str, Any]:
        ...
        return {"total_allocated": total_allocated, "in_use": in_use, "pools": pool_stats}
