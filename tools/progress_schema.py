    ...
    for i, phase in enumerate(data['phases']):
        ...
        if 'status' in phase:
            ...
            if phase['status'] not in valid_statuses:
                errors.append(f"{phase_prefix}: Invalid status '{phase['status']}', must be one of: {', '.join(sorted(valid_statuses))}")
        ...
        for j, task in enumerate(phase['tasks']):
            ...
            if 'status' in task:
                ...
                if task['status'] not in valid_statuses:
                    errors.append(f"{task_prefix}: Invalid status '{task['status']}', must be one of: {', '.join(sorted(valid_statuses))}")
            ...
            if 'priority' in task:
                ...
                if task['priority'] not in valid_priorities:
                    errors.append(f"{task_prefix}: Invalid priority '{task['priority']}', must be one of: {', '.join(sorted(valid_priorities))}")
            ...
            for k, subtask in enumerate(task['subtasks']):
                ...
                if 'status' in subtask:
                    ...
                    if subtask['status'] not in valid_statuses:
                        errors.append(f"{subtask_prefix}: Invalid status '{subtask['status']}', must be one of: {', '.join(sorted(valid_statuses))}")
                ...
                if 'priority' in subtask:
                    ...
                    if subtask['priority'] not in valid_priorities:
                        errors.append(f"{subtask_prefix}: Invalid priority '{subtask['priority']}', must be one of: {', '.join(sorted(valid_priorities))}")
