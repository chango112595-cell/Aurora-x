                    ...
                    if 'function ' in content or 'export default function' in content:
                        ...
                        if "import { ErrorBoundary }" not in content:
                            ...
                            if import_section:
                                ...
                                content = content[:insert_pos] + "import { ErrorBoundary } from '@/components/error-boundary';\n" + content[insert_pos:]
                                fixes.append({
        ...
        if matches:
            for match in matches:
                ...
                if 'messages' in effect_body or 'scroll' in effect_body:
                    content = re.sub(
                        use_effect_pattern,
                        lambda m: m.group(0).replace('}, [])', '}, [messages])') if 'messages' in m.group(1) else m.group(0),
                        content,
                        count=1
                    )
                    if content != original_content:
