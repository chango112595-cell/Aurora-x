...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('/tmp/aurora_supervisor.log'),
...
class AuroraSupervisor:
    ...
    def save_config(self):
        ...
        config_data = {
            'services': [asdict(svc) for svc in self.services.values()]
        }
    ...
    def get_process_for_port(self, port: int) -> Optional[int]:
...
def main():
    ...
    parser.add_argument("command", choices=["start", "stop", "pause", "resume", "restart", "status"], help="Command to execute")
