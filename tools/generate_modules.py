...
def generate_modules(
    dry_run: bool = False, force: bool = False, update_init: bool = False, limit: int | None = None
):
    ...
    for i, mod in enumerate(modules, 1):
        ...
        write_file(
            exec_file,
            build_execute_template(module_id, module_name, category, permissions, sandbox, tags),
            dry_run,
        )

        write_file(clean_file, build_cleanup_template(module_id, module_name, category), dry_run)
...
def main():
    parser = argparse.ArgumentParser(description="Aurora-X Production Module Generator")
    parser.add_argument(
        "--dry-run", action="store_true", help="Show what would be created without creating files"
    )
    parser.add_argument("--force", action="store_true", help="Overwrite existing files")
    parser.add_argument("--update-init", action="store_true", help="Only update __init__.py files")
    parser.add_argument(
        "--limit", type=int, default=None, help="Limit number of modules to generate"
    )

    args = parser.parse_args()

    generate_modules(
        dry_run=args.dry_run, force=args.force, update_init=args.update_init, limit=args.limit
    )
