...
class UltimateAPIManager:
    ...
    def __init__(self, auto_start=True):
        ...
        self.service_architecture = {
            "chat_interface": {
                ...
                "common_issues": ["cors_error", "api_timeout", "invalid_response_format", "synthesis_id_not_found", "empty_response"],
                "auto_fixes": {
            ...
            "comparison_dashboard": {
                ...
                "common_issues": ["empty_data", "permission_denied", "database_connection_error", "spec_generation_failed"],
                "auto_fixes": {
        ...
        self.services = {
            ...
            "aurora_ui": {
                ...
                "routes": [
                    ...
                    {"path": "/chat", "component": "chat-interface.tsx", "purpose": "Chat functionality", "apis": ["learning_api", "bridge_api"]},
                    {"path": "/dashboard", "component": "ComparisonDashboard.tsx", "purpose": "Data comparison", "apis": ["learning_api"]},
                    {"path": "/files", "component": "FileExplorer.tsx", "purpose": "File management", "apis": ["file_server", "bridge_api"]}
                ],
                "api_dependencies": {
                    "learning_api": {
                    ...
                    "file_server": {
                        "endpoints": ["/"],
                        "critical": False,
                        "fallback": "use_direct_file_access"
                    }
                },
                "common_issues": [
            ...
            "learning_api": {
                ...
                "start_cmd": ["python3", "-m", "uvicorn", "aurora_x.serve:app", "--host", "0.0.0.0", "--port", "5002", "--reload"],
                ...
                "api_endpoints": {
                    ...
                    "/api/chat": {"method": "POST", "purpose": "Chat processing", "response": "synthesis_id", "frontend_dependency": "chat_interface"},
                    "/dashboard/spec_runs": {"method": "GET", "purpose": "Dashboard data", "response": "spec run array", "frontend_dependency": "comparison_dashboard"},
            ...
            "bridge_api": {
                ...
                "start_cmd": ["python3", "-m", "uvicorn", "aurora_x.bridge.service:app", "--host", "0.0.0.0", "--port", "5001", "--reload"],
                "cwd": "/workspaces/Aurora-x",
                "technology": "FastAPI/Python",
                ...
                "api_endpoints": {
                    ...
                    "/api/bridge/nl": {"method": "POST", "purpose": "Natural language to project", "response": "generation result", "frontend_dependency": "chat_interface"},
                    "/api/bridge/spec": {"method": "POST", "purpose": "Spec file generation", "response": "spec result", "frontend_dependency": "comparison_dashboard"},
                    "/api/bridge/deploy": {"method": "POST", "purpose": "Deploy to platforms", "response": "deployment result", "frontend_dependency": "file_operations"}
                },
                "file_dependencies": [
            ...
            "file_server": {
                ...
                "file_dependencies": [
                    "/workspaces/Aurora-x/"  # Serves entire project
                ],
                "common_issues": [
        ...
        for dep in service.get("dependencies", []):
            try:
                if dep == "node":
                    ...
                    result = subprocess.run(["python3", "-c", f"import {dep}; print({dep}.__version__)"], 
            ...
            if response.status_code in [200, 404]:
                ...
                if expected_content:
                    ...
                    if expected_content in response_text:
                        ...
                        health_data["alerts"].append(f"Content validation failed: expected '{expected_content}' in response")
    ...
    def comprehensive_auto_heal(self) -> Dict[str, Any]:
        ...
        self.log(f"🎉 Comprehensive healing complete: {healing_results['services_healthy']}/{healing_results['total_services']} services healthy")
        ...
        try:
            ...
            if service["type"] == "frontend":
                ...
                try:
                    ...
                    subprocess.run(["npm", "install"], cwd=service["cwd"], check=True, 
            ...
            for attempt in range(max_attempts):
                ...
                if health["healthy"] and health["content_valid"]:
                    ...
                    print(f"🔄 {service['description']} port listening, waiting for healthy response... ({attempt+1}/{max_attempts})")
                else:
                ...
                for service_name, health in services:
                    ...
                    if health["healthy"]:
                        ...
                        if health.get("content_valid") is False:
                            print(f"      ⚠️  Content validation failed - may be serving wrong content type")
                    else:
        ...
        for service_name in self.services:
            ...
            if health["healthy"]:
                ...
                if service.get("priority") == "critical":
                    scan_results["critical_issues"].append(f"Critical service {service_name} down: {health.get('error', 'Unknown')}")
                else:
                    scan_results["warnings"].append(f"Service {service_name} unhealthy: {health.get('error', 'Unknown')}")
    ...
    def autonomous_decision_engine(self, scan_results: Dict[str, Any]) -> List[str]:
        ...
        slow_services = [name for name, data in scan_results["services"].items() 
    ...
    def _detect_and_fix_imports(self, file_path: Path, content: str) -> Dict[str, List]:
        ...
        language = "python" if file_ext == ".py" else "javascript" if file_ext in [".js", ".jsx"] else "typescript" if file_ext in [".ts", ".tsx"] else "unknown"
        ...
        for i, line in enumerate(lines):
            for problem_import, info in import_problems.items():
                if problem_import in line and "try:" not in lines[max(0, i-2):i+1]:
                    ...
                    fix_lines = [
                        f"            tools_dir = Path(__file__).parent.parent / \"tools\"",
                        f"            sys.path.insert(0, str(tools_dir))",
                        f"            try:",
                        f"                {line.strip()}",
                        f"            except ImportError as e:",
                        f"                raise HTTPException(status_code=500, detail=f\"Failed to import module: {{str(e)}}\")"
                    ]
    ...
    def intelligent_system_analysis(self) -> Dict[str, Any]:
        ...
        code_files = [
            ...
            {"path": Path("/workspaces/Aurora-x/client/src/pages/ComparisonDashboard.tsx"), "type": "frontend", "language": "typescript"},
            {"path": Path("/workspaces/Aurora-x/tools/ultimate_api_manager.py"), "type": "management", "language": "python"},
            {"path": Path("/workspaces/Aurora-x/aurora_x/bridge/attach_bridge.py"), "type": "api", "language": "python"}
        ]
        ...
        for service_name in self.services:
            ...
            if not health["healthy"]:
                analysis_results["critical_issues"].append(f"Service {service_name} is unhealthy: {health.get('error', 'Unknown error')}")
    ...
    def _fix_timeout_error(self, service_name: str, error_details: Dict) -> bool:
        ...
        if self._is_service_overloaded(service_name):
            print(f"   🔄 Service overloaded - restarting...")
            return self._restart_service_intelligent(service_name)
    ...
    def _try_alternative_startup(self, service_name: str) -> bool:
        ...
        alt_commands = {
            "learning_api": ["python", "-m", "uvicorn", "aurora_x.serve:app", "--host", "0.0.0.0", "--port", "5002", "--reload"],
            "bridge_api": ["python", "-m", "uvicorn", "aurora_x.bridge.serve:app", "--host", "0.0.0.0", "--port", "5001", "--reload"],
    ...
    def _request_aurora_assistance(self, service_name: str, issue_type: str, error_details: Dict) -> bool:
        ...
        try:
            ...
            response = requests.post(
                self.aurora_learning_endpoint,
                json={"message": prompt},
                timeout=10
            )
            
            if response.status_code == 200:
                print(f"   🤖 Aurora provided assistance - applying fix...")
                return self._restart_service_intelligent(service_name)
    ...
    def aurora_intelligent_code_assistant(self) -> Dict[str, Any]:
        ...
        if results["learning_insights"]:
            print(f"   📚 Aurora's Learning Insights:")
            ...
            if issue["type"] == "missing_local_module":
                ...
                for i, line in enumerate(lines):
                    if old_import_line in line and "Aurora:" not in line:
                        ...
                        tools_setup = "" if has_tools_setup else '''# Aurora: Intelligent path setup
import sys
from pathlib import Path
tools_dir = Path(__file__).parent.parent / "tools"
...
def main():
    ...
    parser.add_argument('--comprehensive-heal', action='store_true', help='Comprehensive auto-healing with full system knowledge')
    ...
    try:
        if args.status:
            ...
            print(f"   🔍 Issues Detected Categories:")
