#!/usr/bin/env python3
"""
Aurora Universal Start Command - Works on Windows, Linux, macOS
Starts all Aurora services with automatic platform detection.
"""
...
def ensure_python_env(state: dict) -> str:
    ...
    pip_ok = (
        subprocess.run([python_cmd, "-m", "pip", "--version"], capture_output=True).returncode == 0
    )
...
print("[AURORA] Starting ALL systems...")
print(f"   {counts['tiers']} Tiers | {counts['aems']} AEMs | " f"{counts['modules']} Modules")
...
print("\n" + "=" * 80)
print("[DATA] AURORA SYSTEM STATUS")
print("=" * 80)
