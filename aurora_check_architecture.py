...
class AuroraArchitectureChecker:
    ...
    def check_layered_architecture(self):
        ...
        for layer_name, layer_info in layers.items():
            ...
            for path_pattern in layer_info['paths']:
                if '*' in path_pattern:
                    ...
                    for match in matches[:3]:
                        ...
                        layer_info['components'].append(str(match.relative_to(self.root)))
                    if len(matches) > 3:
            ...
            if found == 0:
                ...
                self.architecture['issues'].append(f"{layer_name} layer has no components")
    ...
    def check_service_architecture(self):
        ...
        for service, port in service_port_map.items():
            ...
            if service_file and service_file.exists():
                print(f"        ✅ Implementation: {service_file.relative_to(self.root)}")
            else:
        ...
        if package_json.exists():
            with open(package_json, encoding='utf-8') as f:
                ...
                if proxy:
                    ...
                    print(f"  ✅ Frontend → Backend: Proxy configured ({proxy})")
    ...
    def identify_issues_and_recommendations(self):
        ...
        if 'npm' not in str(self.architecture.get('issues', [])):
            ...
            recommendations.append("Ensure Node.js and npm are installed and in PATH")
        ...
        if missing_services:
            issues.append(f"Missing service implementations: {', '.join(missing_services)}")
        ...
        if self.architecture.get('orchestration', {}).get('orchestrators'):
            orchestrator_count = len(self.architecture['orchestration']['orchestrators'])
            print(f"  ✅ {orchestrator_count} orchestrators configured")
        else:
            issues.append("Orchestration layer incomplete")
            recommendations.append("Ensure all orchestrators are properly configured")
        ...
        if db_files:
            ...
            recommendations.append("Consider adding database for persistent storage")
    ...
    def generate_architecture_report(self):
        ...
        print(f"   Active Ports: {', '.join(map(str, services.get('ports', [])))}")
        ...
        print(f"   Overall Score: {scale.get('score', 0)}/{scale.get('max_score', 0)} ({scale.get('percentage', 0):.1f}%)")
