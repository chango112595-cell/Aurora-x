...
class AuroraDeepSystemUpdater:
    ...
    def generate_patterns(self, counts: Dict[str, int]) -> List[Tuple[re.Pattern, str, str]]:
        ...
        patterns.extend([
            ...
            (re.compile(r'Tier[s]?\s+(\d+)(?![\d])'), lambda m: f'Tiers {tiers}' if int(m.group(1)) != tiers and int(m.group(1)) > 40 else m.group(0), 'Tiers count'),
            ...
            (re.compile(r'total_capabilities["\']?\s*:\s*(\d+)'), f'total_capabilities": {total}', 'total_capabilities JSON'),
            ...
            (re.compile(r'\*\*(\d+)\s+Total\s+Capabilities\*\*'), f'**{total} Total Capabilities**', 'bold capabilities'),
